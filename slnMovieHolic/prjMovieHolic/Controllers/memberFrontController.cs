...
namespace prjMovieHolic.Controllers
{
    public class memberFrontController : SuperFrontController
    {
        ...
        [HttpPost]
        //忘記密碼：驗證是否有此會員
        public IActionResult IsForgetPassword(string email)
        {
            try
            {
                bool isSent = _movieContext.TMembers.Any(o => o.FEmail == email);
                if (!isSent)
                {
                ...
                else
                {
                    CForgetPassword CforgetPassword = new CForgetPassword(_movieContext);
                    CforgetPassword.getNewPasswordEmail(email);
                    return Json(new { success = true, message = "已寄送新密碼至電子郵件。" });
                }
            }
            catch
            {
                return Json(new { success = false, message = "連線異常請重新再試。" });
            }
